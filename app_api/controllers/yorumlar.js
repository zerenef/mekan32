...
const yorumGetir = function(req, res){
  //yorumid parametresi var mı diye kontrol etmeliyiz
  if(req.params && req.params.mekanid && req.params.yorumid){
    //sadece mekan adını ve mekanın yorumlarını getiriyoruz
    ...
    .exec(function(hata, mekan){
      ...
      //mekana ait yorum var mı? kontrol ediyoruz
      if(mekan.yorumlar && mekan.yorumlar.length > 0){
        //yorumid doğru mu? kontrol ediyoruz
        yorum = mekan.yorumlar.id(req.params.yorumid);
        if(!yorum){
          cevapOlustur(res, 404, {'mesaj' : 'yorumid bulunamadı'});
          return;
        } else {
          //yorumid doğruysa cevap nesnesini döndür
          //içine mekanın adını, idsini ve yorumlarını ekle
          cevap = {
            mekan : {
              ad : mekan.ad,
